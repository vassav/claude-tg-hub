...
сессий Claude Code. Код — в [`tg-hub/`](tg-hub/) на `.mjs` (имя папки историческое).
...
([`tg-hub/panel/`](tg-hub/panel/)): зеркало ответов панели в Telegram, инжект сообщений из TG
...
- **fork-plus-build не понадобился** — channels закрыли задачу своим кодом (`tg-hub/`). Из
...
- **Режим VSCode-панели реализован** отдельным путём — `claudeProcessWrapper`-stdio-интерпозер
  (`tg-hub/panel/`), а НЕ channel-инъекцией: в SDK-режиме панели channel-сообщения не запускают
